...
class edit_file_type extends html_form_adapter
{
    public function config()
    {
        ...
        $this->reference= $this->i_var['form_name']= "edit_file_type";

        $this->set_title($t->create." | ".$t->edit, "h2");

        if (($_GET['id'] && ($this->id= $this->require_id("numeric", $_GET['id'])) && $this->id)
                || ($_REQUEST['id_file_type'] && ($this->id= $this->require_id("numeric", $_REQUEST['id_file_type'])) && $this->id)) {
            $this->data_source= "select_file_type1";
            $this->has['update_data_from_global']= false;
            $q->set_filter("id_file_type=".$this->id);
        } else {
            $this->is['new']= true;
        }
    ...
    public function define_form()
    {
        ...
        $this->fields['id_file_type']= $this->id;
    }
    ...
    public function onsubmit()
    {
        ...
        if (($_REQUEST['form_name'] == $this->i_var['form_name'])
            ...
            if ($_REQUEST["create_".$this->subject]) {
                if (!$q->sql_action("insert_file_type1")) {
                    $this->throw_msg(
                        "error",
                        "create_failed",
                                    "#met #onsubmit"
            ...
            if ($_REQUEST["save_".$this->subject]) {
                $q->set_var("id_file_type", $this->id);
                    
                if (!$q->sql_action("delete_from_file_type1") || !$q->sql_action("save_file_type1")) {
                    $this->throw_msg(
                        "error",
                        "save_failed",
                                    "#met #onsubmit"
            ...
            if ($_REQUEST["yes_delete_".$this->subject]) {
                ...
                if (!$q->sql_action("delete_from_file_type1")) {
                    $this->throw_msg(
                        "error",
                        "delete_failed",
                                    "#met #onsubmit"
